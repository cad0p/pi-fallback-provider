...
| Diagnostic log output (`index.ts` logger, `aliases.ts` messages) | Fresh temp `PI_CODING_AGENT_DIR` holding only `auth.json` with one sibling slot (e.g. `{"opencode":{"type":"api_key","key":"sk-test"},"opencode-2":{"type":"api_key","key":"sk-test-2"}}`; test-only keys) and no `pi-fallback-alias-models.json`, then from the worktree: `PI_CODING_AGENT_DIR="$TMP" pi -e "$PWD/index.ts" -p "say ok" 2> "$TMP/stderr.log"`. Assert stderr shows the first-startup line `No cached alias models; aliases will register on session_start` prefixed exactly once (`[pi-fallback-provider]`) with no doubled `[pi-fallback] [pi-fallback`. Only the startup line is asserted: a test key may 401, so a successful model call is not claimed. |
